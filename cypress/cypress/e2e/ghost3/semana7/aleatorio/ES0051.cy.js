...
describe("Crear tag y modificar descripción   ", () => {
  let _siteObject = new Site('ES051');
  let _loginObject = new Login('ES051');
  let _tagObject = new Tag();
  let menuObject = new Menu('ES051');
  ...
  let desc2 = faker.lorem.sentence().substring(0, 50);

  it("Test Crear tag y modificar descripción  ", () => {
    ...
    _tagObject.then_valida_titulo_tag(Tag1);
     ...
     _tagObject.when_retorna_slug ().then((value) => {
      ...
      // AND: usuario limpia descripción del tag
    _tagObject. When_descripcion_color();

    // AND: usuario modifica la descripción del tag
    _tagObject.when_user_description_tag(desc2);
    // AND: usuario guarda tag
    _tagObject.when_user_save_tag ();
    // THEN: valida descripcion tag
    _tagObject.then_valida_descripcion_tag(desc2); 
